        ...
        message.guild.fetchBans().then( bans => {
            ...
            if (user) {
                embed.setTitle(`Successfully Unbanned ${user.user.tag}`)
                    .setColor('#00ff00')
                    .addFields('User ID', user.user.id, true)
                    .addFields('user Tag', user.user.tag, true)
                    .addFields('Banned Reason', user.reason != null ? user.reason : 'no reason')
                    .addFields('Unbanned Reason', reason)
